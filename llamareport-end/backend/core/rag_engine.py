...
class RAGEngine:
    ...
    def __init__(self, storage_dir: str = "./storage", collection_name: str = "documents"):
        ...
        self._retrieval_cache_ttl = int(os.getenv("RETRIEVAL_CACHE_TTL_SECONDS", "900"))
    ...
    def build_index(self, processed_documents: Dict[str, Any], extracted_tables: Dict[str, List[Dict]] = None, incremental: bool = True) -> bool:
        ...
        try:
            ...
            # 对文本做向量化前净化，避免因非法字符导致 embeddings 请求 JSON 解析失败
            all_documents = self._prepare_documents_for_embedding(all_documents)
            if not all_documents:
                logger.error("❌ 所有文档在净化后均不可用于向量化")
                return False
    ...
    def _sanitize_text_for_embedding(self, text: str) -> str:
        """
        清洗文本，确保可以安全放入 JSON 请求体。
        重点处理 PDF/Excel 抽取中常见的控制字符与非法代理字符。
        """
        if text is None:
            return ""
        ...
        # 去掉 UTF-16 surrogate 半个字符（非法 Unicode）
        text = re.sub(r"[\ud800-\udfff]", "", text)
        # 去掉 NUL 与其他不可见控制符（保留 \\n \\r \\t）
    ...
    def _prepare_documents_for_embedding(self, documents: List[Document]) -> List[Document]:
        """
        对文档逐条净化并校验 JSON 可序列化，失败文档跳过并打印来源。
        """
        ...
        for i, doc in enumerate(documents):
            original_text = doc.text if isinstance(doc.text, str) else str(doc.text)
            cleaned_text = self._sanitize_text_for_embedding(original_text)
            ...
            if not cleaned_text:
                ...
                logger.warning(
                    "⚠️ 跳过空文本文档: source_file=%s table_id=%s page=%s idx=%s",
            ...
            # 预检查：确保可放入 JSON body
            try:
                json.dumps({"input": cleaned_text}, ensure_ascii=False)
            except Exception as json_err:
                dropped_count += 1
                meta = doc.metadata or {}
                logger.error(
                    "❌ 跳过非法 JSON 文档: source_file=%s table_id=%s page=%s idx=%s err=%s",
                    meta.get("source_file"),
                    meta.get("table_id"),
                    meta.get("page_number"),
                    i,
            ...
            # Document.text 在部分版本中是只读属性，不能原地修改；改为构造副本
            try:
                if hasattr(doc, "model_copy"):
                    new_doc = doc.model_copy(update={"text": cleaned_text})
                else:
                    raise AttributeError("model_copy unavailable")
            except Exception:
                # 回退：尽量保留 metadata/id
                new_doc_kwargs: Dict[str, Any] = {
                    "text": cleaned_text,
                    "metadata": dict(getattr(doc, "metadata", {}) or {}),
                }
                doc_id = getattr(doc, "doc_id", None) or getattr(doc, "id_", None)
                if doc_id:
                    new_doc_kwargs["doc_id"] = doc_id
                new_doc = Document(**new_doc_kwargs)

            cleaned_docs.append(new_doc)

        if changed_count > 0:
            logger.info("🧹 向量化前文本净化完成: 修复 %s 条文档", changed_count)
        if dropped_count > 0:
            logger.warning("⚠️ 向量化前剔除 %s 条异常文档", dropped_count)
        return cleaned_docs
    ...
    def query(
        ...
        Args:
            ...
            cache_profile: 检索缓存配置标签（仅影响检索缓存键）
        ...
        try:
            ...
            # 只缓存检索证据：context + sources，不缓存最终回答
            retrieval_key = self._build_retrieval_cache_key(question, context_filter, top_k, cache_profile)
            cached_retrieval = self._get_retrieval_cache(retrieval_key)
            if cached_retrieval:
                print("⚡ 命中检索缓存，跳过向量检索")
                retrieval_context = cached_retrieval.get("context", "")
                sources = cached_retrieval.get("sources", [])
...
请基于以上检索到的文档内容回答问题。不要编造信息。"""
                    ...
                    try:
                        ...
                        logger.error(f"LLM生成回答失败（缓存命中分支）: {str(e)}")
                        response = f"基于检索缓存的文档内容：\n\n{retrieval_context[:1000]}..."
                else:
...
请基于以上检索到的文档内容回答问题。**请优先依据【表格数据】部分的数值**，PDF 文本仅作补充；若涉及金额或指标，以表格数据为准。如果文档内容中没有相关信息，请明确说明。"""
                    ...
                    retrieval_context = "\n\n".join([s.get("text", "") for s in sources[:20]])
                    self._set_retrieval_cache(
                        retrieval_key,
                        {
                            "context": retrieval_context,
    ...
    def get_index_version(self) -> str:
        """Best-effort version stamp for index/cache invalidation."""
        try:
            target = self.storage_dir / "chroma"
            ...
            for root, _dirs, files in os.walk(target):
                for f in files:
                    ...
                    try:
                        ...
                        continue
            return f"{int(latest_mtime)}:{self.collection_name}"
        except Exception:
    ...
    def _build_retrieval_cache_key(
        ...
        key_raw = json.dumps(payload, ensure_ascii=False, sort_keys=True)
        return hashlib.sha1(key_raw.encode("utf-8")).hexdigest()

    def _get_retrieval_cache(self, key: str) -> Optional[Dict[str, Any]]:
        now = time.time()
    ...
    def _set_retrieval_cache(self, key: str, value: Dict[str, Any]) -> None:
        with self._retrieval_cache_lock:
            self._retrieval_cache[key] = {
                "value": value,
                "expire_at": time.time() + self._retrieval_cache_ttl,
            }
