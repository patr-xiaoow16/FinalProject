...
from core.query_engine_adapter import CachedQueryEngineAdapter
...
def get_report_agent():
    ...
    if report_agent is None:
        try:
            ...
            # 使用缓存适配器，保证章节工具内 query_engine.query 共享检索池
            cached_query_engine = CachedQueryEngineAdapter(
                raw_query_engine=rag.query_engine,
                retrieval_hub=get_retrieval_hub(),
                scene="agent",
                profile="section_deep",
            )
            report_agent = ReportAgent(cached_query_engine)
...
class GenerateSectionRequest(BaseModel):
    ...
    model_type: Optional[str] = Field(
        default=None,
        description="投资策略模型类型: all（旧综合）、correlation_only（仅相关性）、clustering（仅聚类）、factor_only（仅因子分析）、earnings_forecast（盈利预测）、valuation_anchor（估值锚点分析）、comprehensive_strategy（综合投资策略分析）"
    )
