...
from core.retrieval_hub import RetrievalHub
from core.scene_runners import MetricClickRunner, ChatRunner, LinkageRunner
...
def get_retrieval_hub():
    global retrieval_hub, metric_runner, chat_runner, linkage_runner
    if retrieval_hub is None:
        retrieval_hub = RetrievalHub(get_rag_engine())
        metric_runner = MetricClickRunner(retrieval_hub)
        chat_runner = ChatRunner(retrieval_hub)
        linkage_runner = LinkageRunner(retrieval_hub)
    return retrieval_hub

class QueryRequest(BaseModel):
    question: str
    context_filter: Optional[Dict[str, Any]] = None
    enable_visualization: bool = True  # 是否启用可视化
    query_mode: str = "report"  # report(报告模式) / qa(问答模式)
...
async def ask_question(request: QueryRequest):
    ...
    try:
        ...
        logger.info(f"收到查询: {question[:50]}... (mode={request.query_mode})")

        run_question = question
        if request.query_mode == "qa":
            run_question = (
                "【快速问答模式】请直接回答，控制在3-6行，优先给结论与关键数字，避免冗长分节。"
                f"\n用户问题：{question}"
            )
        ...
        if chat_runner is not None:
            try:
                _ = chat_runner.run(question=run_question, context_filter=request.context_filter, top_k=10)
            except Exception as warmup_error:
                logger.warning(f"共享检索预热失败（忽略，不影响主流程）: {warmup_error}")

        # 获取RAG引擎并执行查询（仍实时生成答案）
        rag_engine = get_rag_engine()
        result = rag_engine.query(run_question, request.context_filter, cache_profile="chat_fast")
...
class DupontAnalysisRequest(BaseModel):
    ...
    force_refresh: Optional[bool] = False  # 是否强制刷新缓存
...
async def generate_comprehensive_analysis(request: ComprehensiveAnalysisRequest):
    ...
    try:
        ...
        # 根据卡片数量选择处理方式
        if linkage_runner is not None and exploration_question:
            try:
                _ = linkage_runner.run(
                    question=exploration_question,
                    context_filter=request.context_filter,
                    top_k=28,
                )
            except Exception as warmup_error:
                logger.warning(f"联动共享检索预热失败（忽略，不影响主流程）: {warmup_error}")
