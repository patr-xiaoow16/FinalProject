...
class VisualizationAgent:
    ...
    async def generate_visualization(
        ...
        try:
            ...
            # 并行分支B：尽早启动洞察草稿生成，与数据提取/图表生成并行执行
            insight_task = asyncio.create_task(
                self._generate_answer_insight_draft(query, answer, sources)
            )
            ...
            if extracted_data.get('multiple') and extracted_data.get('datasets'):
                max_views = max(1, min(int(max_visualizations or 1), 6))
                datasets = extracted_data['datasets'][:max_views]
                logger.info(f"📊 检测到 {len(datasets)} 个数据集，将为每个数据集生成独立视图")
                shared_draft_insights: List[VisualizationInsight] = []
                if insight_task is not None:
                    try:
                        shared_draft_insights = await insight_task
                    except Exception as insight_error:
                        logger.warning(f"并行洞察草稿获取失败: {insight_error}")
                        shared_draft_insights = []
                ...
                for idx, dataset in enumerate(datasets):
                    ...
                    dataset_insights: List[VisualizationInsight] = []
                    if len(datasets) == 1 and insight_task is not None:
                        try:
                            dataset_insights = await insight_task
                        except Exception as insight_error:
                            logger.warning(f"并行洞察草稿获取失败，将回退数据洞察: {insight_error}")
                            dataset_insights = []
                    ...
                    if not dataset_insights and shared_draft_insights:
                        dataset_insights = shared_draft_insights
    ...
    async def _generate_answer_insight_draft(
        self,
        query: str,
        answer: str,
        sources: Optional[List[Dict]] = None
        ...
        try:
            ...
            prompt = f"""请基于以下材料输出一段中文数据洞察（80-120字）：
要求：
1. 至少引用1-2个具体数值或年份；
2. 解释趋势或变化含义；
3. 只输出正文，不要标题和项目符号。
...
回答内容：{answer[:2000]}
补充来源：
{source_snippets if source_snippets else "无"}
"""
...
async def generate_visualization_for_query(
    ...
    max_visualizations: Annotated[int, "最多生成视图数量"] = 3
) -> Dict[str, Any]:
    """
    为查询生成可视化
