...
def _is_summary_incomplete(summary: str) -> bool:
    ...
    if not text:
        return True
    # 常见未完成尾部特征
    if text.endswith(("：", ":", "，", ",", "（", "(", "和", "及")):
        return True
    # 至少应包含三个子段
    required_headers = ["资产负债表", "利润表", "现金流量表"]
    hit_count = sum(1 for header in required_headers if header in text)
    return hit_count < 3


async def _repair_incomplete_summary(
    llm: Any,
    company_name: str,
    year: str,
    summary: str,
    ...
    """摘要不完整时尝试补全；失败则降级返回原文+说明。"""
    if not _is_summary_incomplete(summary):
        return summary
    try:
        repair_prompt = f"""
你是专业财务分析师。下面摘要可能不完整，请基于证据补全成完整段落，保持原风格，不要编造数据。
必须至少包含：资产负债表、利润表、现金流量表、综合判断 四个小段。
...
{evidence_text[:6000]}
"""
        repaired = await llm.achat([
            ChatMessage(role="system", content="只输出完整摘要正文，不要JSON、不要解释。"),
            ChatMessage(role="user", content=repair_prompt),
        ])
        if hasattr(repaired, "message") and hasattr(repaired.message, "content"):
            repaired_text = str(repaired.message.content).strip()
        elif hasattr(repaired, "content"):
            repaired_text = str(repaired.content).strip()
        else:
            repaired_text = str(repaired).strip()
        if repaired_text and not _is_summary_incomplete(repaired_text):
            return repaired_text
    except Exception:
        pass
    return summary.rstrip() + "\n\n（提示：本次输出出现中断，已返回可用核心内容。）"
...
async def generate_financial_review(
    ...
    try:
        ...
        try:
            ...
            cash_flow_data = evidence_text
            logger.info("✅ [generate_financial_review] 聚合证据检索完成（单次检索复用三表）")
...
请严格输出JSON格式，结构如下：
{{
  "summary": "包含资产负债表/利润表/现金流量表/综合判断四段的总结文本",
...
"""
        ...
        summary_value = result_dict.get("summary")
        if isinstance(summary_value, str):
            result_dict["summary"] = await _repair_incomplete_summary(
                llm=llm,
