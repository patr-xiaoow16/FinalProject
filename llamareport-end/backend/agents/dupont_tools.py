...
STANDARD_AMOUNT_UNIT = "百万元"
_DUPONT_CACHE_TTL_SECONDS = 1800
_DUPONT_CACHE_LOCK = Lock()
_context_by_year_cache: Dict[str, Dict[str, Any]] = {}
_metrics_json_cache: Dict[str, Dict[str, Any]] = {}
_normalized_financial_cache: Dict[str, Dict[str, Any]] = {}
_numeric_dupont_cache: Dict[str, Dict[str, Any]] = {}


def _build_dupont_cache_key(
    cache_type: str,
    company_name: str,
    year: str,
    ...
    payload = {
        ...
        "unit": STANDARD_AMOUNT_UNIT,
        "pipeline_version": "dupont_v2",
    }
    if extra:
...
def _cache_get(store: Dict[str, Dict[str, Any]], key: str) -> Optional[Any]:
    with _DUPONT_CACHE_LOCK:
        item = store.get(key)
        if not item:
            return None
        if time.time() > item.get("expire_at", 0):
            store.pop(key, None)
            return None
        return item.get("value")


def _cache_set(store: Dict[str, Dict[str, Any]], key: str, value: Any, ttl: int = _DUPONT_CACHE_TTL_SECONDS) -> None:
    with _DUPONT_CACHE_LOCK:
        store[key] = {
            "value": value,
...
def _cache_file_path(cache_type: str, cache_key: str) -> Path:
    cache_dir = Path(__file__).parent.parent / "storage" / "dupont_cache" / cache_type
    cache_dir.mkdir(parents=True, exist_ok=True)
    return cache_dir / f"{cache_key}.json"


def _disk_cache_get(cache_type: str, cache_key: str) -> Optional[Any]:
    path = _cache_file_path(cache_type, cache_key)
    if not path.exists():
        return None
    try:
        ...
        if expire_at and time.time() > float(expire_at):
            try:
        ...
        logger.warning(f"读取磁盘缓存失败({cache_type}): {e}")
        return None


def _disk_cache_set(cache_type: str, cache_key: str, value: Any, ttl: int = _DUPONT_CACHE_TTL_SECONDS) -> None:
    path = _cache_file_path(cache_type, cache_key)
    try:
        with open(path, "w", encoding="utf-8") as f:
            json.dump(
                {"expire_at": time.time() + ttl, "value": value},
                f,
                ensure_ascii=False,
                indent=2,
            )
    except Exception as e:
        logger.warning(f"写入磁盘缓存失败({cache_type}): {e}")
...
def _to_standard_amount(value: Optional[float], unit: Optional[str]) -> Optional[float]:
    """将金额统一到百万元口径。"""
    yuan_val = _to_yuan(value, unit)
    if yuan_val is None:
        return None
    return yuan_val / 1e6


def _normalize_amount_financial_data(financial_data: Dict[str, float], assume_yuan: bool = False) -> Dict[str, float]:
    amount_keys = {"净利润", "营业收入", "总资产", "股东权益", "流动资产", "非流动资产", "营业利润", "总负债"}
    normalized: Dict[str, float] = {}
    for k, v in (financial_data or {}).items():
        if v is None:
            continue
        try:
            fv = float(v)
            ...
            continue
        if k in amount_keys:
            normalized[k] = fv / 1e6 if assume_yuan else fv
        else:
            normalized[k] = fv
    return normalized


def _is_reasonable_prev_year_value(
    metric_name: str,
    prev_value: Optional[float],
    prev_unit: Optional[str],
    curr_value: Optional[float],
    curr_unit: Optional[str],
) -> bool:
    ...
    if metric_name in {"ROE", "ROA"}:
        return -100.0 <= pv <= 100.0
    if metric_name in {"NetProfit", "Revenue", "TotalAssets", "Equity"}:
        prev_yuan = _to_yuan(pv, prev_unit)
        if prev_yuan is None or prev_yuan <= 0:
            return False
        if curr_value is not None:
            curr_yuan = _to_yuan(float(curr_value), curr_unit)
            if curr_yuan and curr_yuan > 0:
                ratio = prev_yuan / curr_yuan
                # 对上一年金额做宽松量级门禁，过滤明显误提取（如 1.8e-05）
...
def _supplement_previous_year_metrics(
    query_engine,
    company_name: str,
    main_year: int,
    ...
    existing = {
        m.get("metric")
        for m in metrics
        if m.get("year") == prev_year and m.get("value") is not None
    }
    missing_defs = [d for d in metric_defs if d["metric"] in {"ROE", "ROA", "NetProfit", "Revenue", "TotalAssets", "Equity"} and d["metric"] not in existing]
    if not missing_defs:
        return

    prev_ctx = (context_by_year or {}).get(prev_year) or context_text
    if not prev_ctx:
        return

    # 1) 规则补抽（上年专用）
    for metric_def in missing_defs[:]:
        metric_name = metric_def["metric"]
        v, u, s = _extract_metric_by_year_from_text(prev_ctx, metric_def["aliases"], prev_year, metric_def["type"])
        if v is None and metric_def["type"] == "amount":
            # 针对“上年/同期”列再试一次（report_year=main_year）
            yeared = _extract_yeared_metrics_from_table(prev_ctx, [metric_def], report_year=main_year)
            slot = yeared.get(metric_name, {}).get(prev_year)
            if slot:
                v, u, s = slot
        curr_item = next((m for m in metrics if m.get("metric") == metric_name and m.get("year") == main_year), None)
        curr_v = curr_item.get("value") if curr_item else None
        curr_u = curr_item.get("unit") if curr_item else None
        if _is_reasonable_prev_year_value(metric_name, v, u, curr_v, curr_u):
            metrics.append({
                "metric": metric_name,
                "year": prev_year,
                "value": v,
                "unit": u or ("%" if metric_def["type"] == "percent" else "元"),
                "source": s or metric_def["aliases"][0],
                "yoy": None,
            })
            missing_defs.remove(metric_def)
    ...
    try:
        ...
        missing_fields = [field_map[d["metric"]] for d in missing_defs if field_map.get(d["metric"])]
        if not missing_fields:
            return
        missing_text = "、".join(missing_fields)
        prompt = f"""
请仅提取 {company_name} {prev_year} 年（上年/同期）以下缺失指标：{missing_text}。
要求：
1) 只提取 {prev_year} 年，不得使用 {main_year} 年；
2) 金额字段按“元”返回；百分比按数值返回（如10.08代表10.08%）；
3) 找不到返回 null；只输出结构化结果。
...
"""
        ...
        resp = sllm.chat([
            ChatMessage(role="system", content="你是财务指标抽取助手，只能抽取指定年份数据。"),
            ChatMessage(role="user", content=prompt),
        ])
        raw = getattr(resp, "raw", resp)
        data = raw.model_dump() if hasattr(raw, "model_dump") else (raw if isinstance(raw, dict) else {})
        for metric_def in missing_defs:
            metric_name = metric_def["metric"]
            ...
            curr_v = curr_item.get("value") if curr_item else None
            curr_u = curr_item.get("unit") if curr_item else None
            candidate_unit = "%" if metric_def["type"] == "percent" else "元"
            if not _is_reasonable_prev_year_value(metric_name, fv, candidate_unit, curr_v, curr_u):
                continue
            metrics.append({
                "metric": metric_name,
                "year": prev_year,
                "value": fv,
                "unit": candidate_unit,
                "source": f"{field}(prev_year_fallback)",
                "yoy": None,
            })
    except Exception as e:
        logger.warning(f"上一年指标单次兜底补抽失败: {e}")


def _build_financial_data_from_metrics_json(
    ...
    amount_metrics = {"NetProfit", "Revenue", "TotalAssets", "Equity"}
    out: Dict[str, float] = {}
    try:
        target_year = int(year)
    except (ValueError, TypeError):
        target_year = None
    for item in metrics_json.get("metrics", []) or []:
        ...
        if target_year is not None and item_year is not None:
            try:
...
async def generate_dupont_analysis(
    ...
    try:
        ...
        # 阶段A：仅抽取结构化指标并落库（允许检索）
        if financial_data is None:
            financial_data, structured_metrics = await _stage_a_extract_and_persist(
                company_name=company_name,
                year=year,
            ...
            financial_data = _normalize_amount_financial_data(financial_data, assume_yuan=True)
            structured_metrics = None

        # 阶段B：只读取阶段A产物，禁止再检索原文
        result_dict = await _stage_b_analyze_from_structured(
            company_name=company_name,
            year=year,
            filename=filename,
        ...
        logger.info(f"杜邦分析生成成功: ROE={result_dict.get('level1', {}).get('roe', {}).get('formatted_value', '—')}")
        return result_dict
...
async def _stage_a_extract_and_persist(
    company_name: str,
    year: str,
    query_engine,
    filename: Optional[str] = None,
    ...
    metrics_key = _build_dupont_cache_key("metrics_json", company_name, year, filename)
    financial_key = _build_dupont_cache_key("normalized_financial_data", company_name, year, filename)

    cached_metrics = None if force_refresh else (_cache_get(_metrics_json_cache, metrics_key) or _disk_cache_get("metrics_json", metrics_key))
    cached_financial = None if force_refresh else (_cache_get(_normalized_financial_cache, financial_key) or _disk_cache_get("normalized_financial_data", financial_key))
    if cached_metrics is not None:
        _cache_set(_metrics_json_cache, metrics_key, cached_metrics)
    if cached_financial is not None:
        _cache_set(_normalized_financial_cache, financial_key, cached_financial)

    if cached_financial and isinstance(cached_financial, dict):
        return cached_financial, cached_metrics if isinstance(cached_metrics, dict) else None

    if isinstance(cached_metrics, dict) and cached_metrics.get("metrics"):
        derived_financial = _build_financial_data_from_metrics_json(cached_metrics, year)
        if derived_financial:
            _cache_set(_normalized_financial_cache, financial_key, derived_financial)
            _disk_cache_set("normalized_financial_data", financial_key, derived_financial)
            return derived_financial, cached_metrics
    ...
    # 阶段A输出统一金额单位到百万元，避免后续模型口径混乱
    financial_data = _normalize_amount_financial_data(financial_data, assume_yuan=False)

    _cache_set(_normalized_financial_cache, financial_key, financial_data)
    _disk_cache_set("normalized_financial_data", financial_key, financial_data)
    if structured_metrics and structured_metrics.get("metrics"):
        _cache_set(_metrics_json_cache, metrics_key, structured_metrics)
        _disk_cache_set("metrics_json", metrics_key, structured_metrics)
    return financial_data, structured_metrics


async def _stage_b_analyze_from_structured(
    company_name: str,
    year: str,
    ...
    numeric_key = _build_dupont_cache_key("numeric_dupont_result", company_name, year, filename)
    cached_numeric = None if force_refresh else (_cache_get(_numeric_dupont_cache, numeric_key) or _disk_cache_get("numeric_dupont_result", numeric_key))
    if isinstance(cached_numeric, dict):
        result_dict = copy.deepcopy(cached_numeric)
    else:
        dupont_result = analyzer.calculate_dupont_analysis(
            financial_data=financial_data,
            company_name=company_name,
            report_year=year,
            amount_unit=STANDARD_AMOUNT_UNIT,
        )
        ...
        # 只缓存数值结果，不缓存洞察
        cache_numeric_payload = copy.deepcopy(result_dict)
        for k in ("insights", "strengths", "weaknesses", "recommendations"):
            cache_numeric_payload.pop(k, None)
        _cache_set(_numeric_dupont_cache, numeric_key, cache_numeric_payload)
        _disk_cache_set("numeric_dupont_result", numeric_key, cache_numeric_payload)
...
async def extract_financial_data_for_dupont(
    ...
    try:
        ...
        if retriever and years_to_fetch:
            context_key = _build_dupont_cache_key(
                "context_by_year",
                company_name=company_name,
                year=year,
                filename=filename,
                extra={"years_to_fetch": years_to_fetch, "max_chunks_per_year": 10},
            )
            cached_context = None if force_refresh else (_cache_get(_context_by_year_cache, context_key) or _disk_cache_get("context_by_year", context_key))
            if not force_refresh and isinstance(cached_context, dict) and cached_context:
                context_by_year = cached_context
                _cache_set(_context_by_year_cache, context_key, cached_context)
            else:
                context_by_year = _retrieve_context_by_year(
                    retriever, company_name, years_to_fetch, filename=filename, max_chunks_per_year=10
                )
                _cache_set(_context_by_year_cache, context_key, context_by_year)
                _disk_cache_set("context_by_year", context_key, context_by_year)
        ...
        if structured_metrics.get("metrics"):
            ...
            for metric in structured_metrics["metrics"]:
                ...
                if target_key and metric.get("value") is not None:
                    financial_data[target_key] = float(metric["value"])
        
        # 第五步：数据验证和补充
        financial_data = validate_and_complement_financial_data(financial_data, context_text)
        has_structured_metrics = bool(structured_metrics and structured_metrics.get("metrics"))
        financial_data = _normalize_amount_financial_data(financial_data, assume_yuan=not has_structured_metrics)
...
def _build_structured_metrics_json(
    ...
    # 上一年专用补抽：仅对 year-1 缺失字段触发，不影响主年流程
    if year_value:
        _supplement_previous_year_metrics(
            query_engine=query_engine,
            company_name=company_name,
            main_year=year_value,
    ...
    # 第二步：金额单位统一为百万元（比例/倍数保持不变）
    for item in metrics:
        if item.get("metric") in {"NetProfit", "Revenue", "TotalAssets", "Equity"}:
            normalized = _to_standard_amount(item.get("value"), item.get("unit"))
            if normalized is not None:
                item["value"] = normalized
                item["unit"] = STANDARD_AMOUNT_UNIT
    ...
    for year in years:
        ...
        level2 = {
            ...
            "total_assets": {
                ...
                "level": 2,
                "formula": "总资产",
                "unit": total_assets.get("unit") if total_assets else STANDARD_AMOUNT_UNIT
            },
            "shareholders_equity": {
                "name": "股东权益",
                ...
                "level": 2,
                "formula": "股东权益",
                "unit": equity.get("unit") if equity else STANDARD_AMOUNT_UNIT
            }
        }
        ...
        level3 = {
            "net_income": {
                ...
                "level": 3,
                "formula": "净利润",
                "unit": net_profit.get("unit") if net_profit else STANDARD_AMOUNT_UNIT
            },
            "revenue": {
                "name": "营业收入",
                ...
                "level": 3,
                "formula": "营业收入",
                "unit": revenue.get("unit") if revenue else STANDARD_AMOUNT_UNIT
            },
            "current_assets": {
                "name": "流动资产",
                "value": None,
