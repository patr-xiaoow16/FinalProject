...
# 仅缓存“检索数据文本”，不缓存任何 LLM 洞察或可视化结果
_financial_data_cache: Dict[str, Dict[str, Any]] = {}
_financial_data_cache_lock = Lock()
_financial_data_cache_ttl = int(os.getenv("FINANCIAL_DATA_CACHE_TTL_SECONDS", "1800"))


def _build_financial_cache_key(seed: Dict[str, Any]) -> str:
    payload = json.dumps(seed, ensure_ascii=False, sort_keys=True)
    return hashlib.sha1(payload.encode("utf-8")).hexdigest()[:24]


def _get_financial_cached_data(cache_key: str) -> Optional[str]:
    now = time.time()
    with _financial_data_cache_lock:
        item = _financial_data_cache.get(cache_key)
        if not item:
            return None
        if now - item.get("ts", 0) > _financial_data_cache_ttl:
            _financial_data_cache.pop(cache_key, None)
            return None
        return item.get("data")


def _set_financial_cached_data(cache_key: str, data: str) -> None:
    if not isinstance(data, str) or not data.strip():
        return
    if data.startswith("检索失败"):
        return
    with _financial_data_cache_lock:
        _financial_data_cache[cache_key] = {"ts": time.time(), "data": data}
...
def retrieve_financial_data(
    ...
    try:
        ...
        # 执行查询 - 财务点评优先走较小 top_k，降低延迟
        metric_top_k = {
            "balance_sheet_detailed": 12,
            "income_statement_detailed": 10,
            "cash_flow_detailed": 8,
        }.get(metric_type, 10)
        ...
        if cached is not None:
            ...
            logger.info(
                f"⚡ [retrieve_financial_data] 缓存命中: {metric_type}，耗时: {elapsed:.2f}秒"
            )
            return cached
        ...
        try:
            ...
            if hasattr(query_engine, 'query'):
                try:
                    response = query_engine.query(query, top_k=metric_top_k)
                except TypeError:
                    # 兼容不支持 top_k 参数的 query 实现
            ...
            final_content = content if content else f"未找到{metric_type}相关数据"
            _set_financial_cached_data(cache_key, final_content)
...
def retrieve_financial_evidence_bundle(
    company_name: str,
    year: str,
    query_engine: Any,
    top_k: int = 15,
    ...
    """
    一次性检索财务点评所需证据（仅检索，不触发子LLM回答）。

    优先走 HybridRetriever 的 table_first 策略，拿到表格/文本证据后直接拼接返回。
    """
    ...
    if cached is not None:
        ...
        logger.info(
            f"⚡ [retrieve_financial_evidence_bundle] 缓存命中，耗时: {elapsed:.2f}秒"
        )
        return cached

    try:
        # 优先使用共享 RAG 引擎的 retriever，避免 query() 触发额外 LLM 回答
        ...
        if rag_engine is not None:
            ...
            if (
                ...
                results = [
                    {"document": node, "semantic_score": getattr(node, "score", 0.0)}
                    for node in nodes
                ]
            ...
            if results:
                ...
                if parts:
                    elapsed = time.time() - start_time
                    final_text = "\n\n".join(parts)
                    _set_financial_cached_data(cache_key, final_text)
                    logger.info(
                        f"✅ [retrieve_financial_evidence_bundle] 检索成功，证据片段: {len(parts)}，耗时: {elapsed:.2f}秒"
                    )
                    return final_text
        ...
        if hasattr(query_engine, "query"):
            ...
            elapsed = time.time() - start_time
            logger.warning(
                f"⚠️ [retrieve_financial_evidence_bundle] 使用兼容路径返回内容，耗时: {elapsed:.2f}秒"
            )
            final_content = content if content else "未检索到可用证据"
            _set_financial_cached_data(cache_key, final_content)
            return final_content
