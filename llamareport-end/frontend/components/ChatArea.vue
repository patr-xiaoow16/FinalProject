      ...
      <div class="chat-container">
        ...
        <div class="chat-messages" ref="messagesContainer">
          <div v-for="(msg, index) in messages" :key="index" :class="['chat-message', msg.type, { 'processing-summary': isProcessingSummary(msg.content) }]" @mouseenter="hoveredMessageIndex = index" @mouseleave="hoveredMessageIndex = null">
            ...
            <div v-if="msg.sourceCitation" class="message-citation">{{ msg.sourceCitation }}</div>
            <div v-if="msg.sources && msg.sources.length > 0 && !msg.hideSourcesList" class="message-sources">
        ...
        <div class="chat-input-area">
          <div class="chat-actions">
            <button
              ...
              :disabled="loading"
              :title="queryMode === 'report' ? '当前是报告模式，点击切换到问答模式' : '当前是问答模式，点击切换到报告模式'"
            >
              模式切换
            </button>
          ...
          <div class="chat-input-wrapper">
            <textarea ref="input" v-model="inputText" class="chat-input" :placeholder="inputPlaceholder" rows="1"></textarea>
...
export default {
  ...
  props: { 
    ...
    queryMode: { type: String, default: 'qa' }
  },
  emits: ['send-message', 'clear-chat', 'agent-query', 'agent-analysis', 'dupont-analysis', 'get-suggestions', 'delete-message', 'quick-analysis', 'update-query-mode'],
  data() { 
    return { 
      inputText: '', 
  ...
  computed: {
    inputPlaceholder() {
      if (this.queryMode === 'qa') {
        return '问答模式：快速回答并生成1张图（如：平安银行净利润近3年趋势如何？）';
      }
      return '报告模式：生成更完整的分析回答（如：请分析XX公司2023年的业绩指引）';
    }
  },
  methods: {
    toggleQueryMode() {
      if (this.loading) return;
      const nextMode = this.queryMode === 'report' ? 'qa' : 'report';
      this.$emit('update-query-mode', nextMode);
    },
    ...
    sendMessage() {
      ...
      this.$emit('agent-query', { question, mode: this.queryMode });
    },
    ...
    parseMarkdown(text) {
      ...
      // 2. 按中文序号换行：仅在普通文本行生效，避免破坏 Markdown 标题/加粗（如 "## **一、分析洞察**"）
      out = out.split('\n').map((line) => {
        const trimmed = line.trim();
        // 跳过 Markdown 标题、列表、表格、代码块围栏
        if (
          /^#{1,6}\s/.test(trimmed) ||
          /^(\*|-|\d+\.)\s+/.test(trimmed) ||
          /^\|.*\|$/.test(trimmed) ||
