...
const App = {
  setup() {
    ...
    const queryMode = ref('qa') // report(报告模式) / qa(问答模式)
    ...
    const resolveQueryPayload = (input) => {
      if (typeof input === 'string') {
        return { question: input, mode: queryMode.value }
      }
      if (input && typeof input === 'object') {
        return {
          question: String(input.question || '').trim(),
          mode: input.mode === 'qa' ? 'qa' : 'report'
        }
      }
      return { question: '', mode: queryMode.value }
    }

    const buildContextFilter = () => (
      selectedFile.value && selectedFile.value.filename
        ? { filename: selectedFile.value.filename }
        : null
    )

    const extractPageNumber = (source = {}) => {
    ...
    const collectFileLabel = (sources = []) => {
      for (const source of sources) {
        const metadata = source?.metadata || {}
        const file = metadata.file_name || metadata.filename || metadata.source_file || metadata.source
        if (file) {
          return String(file).replace(/\.(pdf|PDF|xlsx|xls)$/g, '')
        }
      }
      return '年报'
    }
    ...
    const compressPageRanges = (pages = []) => {
      const unique = [...new Set(pages.filter(p => Number.isFinite(p) && p > 0))].sort((a, b) => a - b)
    ...
    const buildCompactCitation = (sources = []) => {
      if (!Array.isArray(sources) || sources.length === 0) return ''
      const fileLabel = collectFileLabel(sources)
      const pages = sources
        .map(extractPageNumber)
        .filter(v => Number.isFinite(v))
      const pageRanges = compressPageRanges(pages)
      if (!pageRanges.length) return `来源：${fileLabel}`
      const limited = pageRanges.slice(0, 5)
      const suffix = pageRanges.length > 5 ? '等' : ''
    ...
    const addVisualizationCards = (visualization, question, singleOnly = false) => {
      ...
      if (visualization.type === 'financial_tables' && Array.isArray(visualization.tables)) {
        const tables = singleOnly ? visualization.tables.slice(0, 1) : visualization.tables
        tables
          .filter(table => table)
          .forEach((table, idx) => {
      ...
      if (Array.isArray(visualization.visualizations) && visualization.visualizations.length > 0) {
        const views = singleOnly ? visualization.visualizations.slice(0, 1) : visualization.visualizations
        views.forEach((viz, idx) => {
          visualizationCards.value.push({
            id: `${now}-chart-${idx}`,
            question: viz.title || question + (views.length > 1 ? ` (${idx + 1})` : ''),
            timestamp: new Date(),
            data: {
              has_visualization: true,
    ...
    const handleQaModeQuery = async (question) => {
      ...
      try {
        ...
        const response = await fetch('/query/ask', {
          ...
          body: JSON.stringify({ 
            ...
            context_filter: buildContextFilter()
          }),
        ...
        if (result.answer) {
          ...
          chatMessages.value.push({
            ...
            sources: sourceList,
            sourceCitation: buildCompactCitation(sourceList),
            hideSourcesList: true,
            timestamp: new Date()
          })
        } else {
          chatMessages.value.push({
        ...
        const added = addVisualizationCards(result.visualization, question, true)
        if (added === 0) {
          showMessage('warning', '问答模式未提取到可视化数据，仅返回文本回答')
        }
      } catch (error) {
        console.error('问答模式查询错误:', error)
        
        let errorMsg = '网络错误或请求超时'
        if (error.name === 'AbortError') {
          errorMsg = '请求超时（超过3分钟），请稍后重试或切换报告模式'
    ...
    const handleReportModeQuery = async (question) => {
      chatMessages.value.push({ type: 'user', content: question, timestamp: new Date() })
      queryLoading.value = true
      try {
        const controller = new AbortController()
    ...
    const handleAgentQuery = async (input) => {
      const payload = resolveQueryPayload(input)
      const question = payload.question
      const mode = payload.mode || queryMode.value
      if (!question) return
      if (mode === 'qa') {
        return await handleQaModeQuery(question)
      }
      return await handleReportModeQuery(question)
    }

    const handleQueryModeChange = (mode) => {
      const nextMode = mode === 'qa' ? 'qa' : 'report'
      queryMode.value = nextMode
      showMessage('success', nextMode === 'qa' ? '已切换到问答模式（快速）' : '已切换到报告模式')
    }
    ...
    return {
      ...
      quickOverviewData, overviewForFilename, queryMode,
      ...
      handleGenerateReport, handleGenerateSection, handleClearChat, checkIndexStatus, loadQuickOverview, handleQueryModeChange,
